...
impl NonBlockingStream {
    ...
    pub fn wants_read(&self) -> bool {
        match self {
            Self::Client(stream) => stream.conn.wants_read(),
            Self::Server(stream) => stream.conn.wants_read(),
        }
    }
    ...
    pub fn read_tls(&mut self) -> io::Result<usize> {
        match self {
            Self::Client(stream) => stream.conn.read_tls(&mut stream.sock),
            Self::Server(stream) => stream.conn.read_tls(&mut stream.sock),
        }
    }

    pub fn process_new_packets(&mut self) {
        match self {
            Self::Client(stream) => stream.conn.process_new_packets().unwrap(),
            Self::Server(stream) => stream.conn.process_new_packets().unwrap(),
        };
    }
}
...
impl Read for NonBlockingStream {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        match self {
            Self::Client(stream) => stream.conn.reader().read(buf),
            Self::Server(stream) => stream.conn.reader().read(buf),
        }
    }
