use std::{
    ...
    sync::{
        Mutex,
        mpsc::{Receiver, RecvError, Sender, SyncSender, TryRecvError, channel, sync_channel},
    },
...
use super::{CommChannel, NetSerializable, non_blocking::NonBlockingCommChannel, receiver};
...
impl ReadTask {
    pub fn new(length: usize, mailback: oneshot::Sender<Vec<u8>>) -> Self {
        Self {
            ...
            length,
            offset: 0,
            mailback: ReadTaskType::Regular(mailback),
        }
    }

    pub fn new_buffered(length: usize, thread_id: u64) -> Self {
        Self {
            buffer: vec![0u8; length],
            length,
            offset: 0,
            mailback: ReadTaskType::Buffered(thread_id),
        }
    }
}
...
enum BufferedRead {
    InProgress,
    InProgressWithMailback(oneshot::Sender<Vec<u8>>),
    Completed(Vec<u8>),
}

struct WriteTask {
    /// thread_id and thread_id_offset
    thread_id: Option<([u8; U64_BYTE_SIZE * 2], usize)>,
    buffer: Vec<u8>,
    offset: usize,
}
...
impl WriteTask {
    pub fn new(thread_id: Option<u64>, buffer: Vec<u8>) -> Self {
        Self {
            thread_id: thread_id.map(|id| {
                let thread_id_bytes = id.to_le_bytes();
                let length_bytes = (buffer.len() as u64).to_le_bytes();
                let mut thread_id_buffer = [0u8; 16];
                thread_id_buffer[..8].copy_from_slice(&thread_id_bytes);
                thread_id_buffer[8..].copy_from_slice(&length_bytes);
                (thread_id_buffer, 0)
            }),
            buffer,
            offset: 0,
...
struct IoThreadContext {
    ...
    thread_buffered_reads: BTreeMap<u64, VecDeque<BufferedRead>>,
...
impl IoThreadContext {
    pub fn new(
        ...
        Ok((
            Self {
                ...
                thread_buffered_reads: BTreeMap::new(),
    ...
    fn handle_io(&mut self, my_direction: Direction) -> io::Result<()> {
        let mut thread_id_buf = [0u8; U64_BYTE_SIZE * 2];
            ...
            match self.state {
                            ...
                            if self.state.is_working() {
                                ...
                                if !self.read_queue.is_empty() {
                                    Self::non_blocking_read(
                                        &mut self.comm,
                                        &mut self.read_queue,
                                        &mut self.thread_buffered_reads,
                                        &mut thread_id_buf,
                ...
                State::Working {
                        ...
                        && !self.comm.stream.wants_write()
                    {
                        ...
                        if self.comm.stream.wants_read() {
                            Self::non_blocking_read_tls(&mut self.comm)?;
                        }
        ...
        match task {
            Task::Read {
                ...
                if let Some(thread_id) = thread_id
                    && let Some(q) = self.thread_buffered_reads.get_mut(&thread_id)
                    && !q.is_empty()
                {
                    if let BufferedRead::InProgressWithMailback(_) = q.front().unwrap() {
                        // We have already caught up with the buffered reads. This one is new and should
                        // be put in the queue as usual
                        self.read_queue
                            .put(Some(thread_id), ReadTask::new(length, mailback));
                    } else {
                        match q.pop_front().unwrap() {
                            BufferedRead::InProgress => {
                                // The task is already in the main queue, no need to enqueue again
                                q.push_front(BufferedRead::InProgressWithMailback(mailback));
                            }
                            BufferedRead::Completed(buffer) => {
                                assert_eq!(length, buffer.len());
                                mailback.send(buffer).expect("Receiver dropped");
                            }
                            BufferedRead::InProgressWithMailback(_) => unreachable!(),
                        }
                        if q.is_empty() {
                            self.thread_buffered_reads.remove(&thread_id);
                        }
                    }
                } else {
                    self.read_queue
    ...
    fn non_blocking_read(
        ...
        thread_buffered_reads: &mut BTreeMap<u64, VecDeque<BufferedRead>>,
        thread_id_buf: &mut [u8; U64_BYTE_SIZE * 2],
                ...
                match channel.stream.read(buf) {
                    Ok(n) => {
                        ...
                        if read_task.offset >= read_task.length {
                            ...
                            match t.mailback {
                                ReadTaskType::Regular(mailback) => mailback
                                    .send(t.buffer)
                                    .expect("Cannot send read result back; receiver was dropped."),
                                ReadTaskType::Buffered(thread_id) => {
                                    let buffered_read_queue =
                                        thread_buffered_reads.get_mut(&thread_id).unwrap();

                                    let should_invoke_mailback =
                                        if let BufferedRead::InProgressWithMailback(_) =
                                            &buffered_read_queue[0]
                                        {
                                            true
                                        } else {
                                            false
                                        };
                                    if should_invoke_mailback {
                                        let buffered_read =
                                            buffered_read_queue.pop_front().unwrap();
                                        if let BufferedRead::InProgressWithMailback(mailback) =
                                            buffered_read
                                        {
                                            mailback.send(t.buffer)
                                                    .expect("Cannot send read result back; receiver was dropped.");
                                        } else {
                                            unreachable!();
                                        }
                                    } else {
                                        for buffered_read in buffered_read_queue {
                                            match buffered_read {
                                                BufferedRead::InProgress => {
                                                    *buffered_read =
                                                        BufferedRead::Completed(t.buffer);
                                                    break;
                                                }
                                                BufferedRead::InProgressWithMailback(_) => {
                                                    panic!(
                                                        "Unexpected InProgressWithMailback in the middle of the queue"
                                                    );
                                                }
                                                BufferedRead::Completed(_) => {}
                                            }
                                        }
                                    }
                                }
                ...
                if !read_task_queue.is_empty() {
                    ...
                    if *thread_id_buf_offset >= thread_id_buf.len() {
                        *thread_id_buf_offset = 0;
                        ...
                        let (chunks, _) = thread_id_buf.as_chunks::<8>();
                        let thread_id = u64::from_le_bytes(chunks[0]);
                        let size = u64::from_le_bytes(chunks[1]);
                        let t = read_task_queue.pop_with_thread_id(thread_id);
                        if let Some(task) = t {
                            read_task_queue.put(None, task);
                        } else {
                            thread_buffered_reads
                                .entry(thread_id)
                                .or_insert_with(|| VecDeque::new())
                                .push_back(BufferedRead::InProgress);
                            read_task_queue
                                .put(None, ReadTask::new_buffered(size as usize, thread_id));
                        }
                    }
                    Ok(())
                } else {
    ...
    fn non_blocking_read_tls(channel: &mut NonBlockingCommChannel) -> io::Result<()> {
        match channel.stream.read_tls() {
            Ok(_) => {
                channel.stream.process_new_packets();
                Ok(())
            } // here we can ignore the reported number of written bytes since they have been counted before in [Self::non_blocking_write]
            Err(io_err) => {
                // a few error types are expected, and are not an error
                if io_err.kind() == ErrorKind::WouldBlock || io_err.kind() == ErrorKind::Interrupted
                {
...
impl IoLayerOwned {
    ...
    pub fn send_field_slice<F: NetSerializable>(&self, direction: Direction, elements: &[F]) {
        self.task_layer
            .get()
            .unwrap()
            .send_field_slice(direction, elements)
    }
...
mod test {
    ...
    use itertools::Itertools;
    use rand::{CryptoRng, Rng, RngCore, seq::SliceRandom, thread_rng};

    use crate::rep3_core::{
        network::{CommChannel, receiver::VecReceiver, task::U64_BYTE_SIZE},
    ...
    fn send_receive_threaded_one() {
        ...
        for _ in 0..(N_THREADS / 2) {
            let mut buf = vec![0u8; N + U64_BYTE_SIZE * 2];
        ...
        for _ in 0..(N_THREADS / 2) {
            ...
            comm_prev_receiver
                ...
                .unwrap();
            comm_prev_receiver
                .write(&(data.len() as u64).to_le_bytes())
                .unwrap();
        ...
        for (id, expected) in data_send {
            ...
            assert_eq!(expected, actual[U64_BYTE_SIZE * 2..]);
        }
