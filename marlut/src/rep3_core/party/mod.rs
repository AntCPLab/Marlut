...
pub trait Party {
    ...
    fn generate_random_local<T: RngExt>(&mut self, n: usize) -> Vec<T>;

    fn generate_random_local_aligned<T: RngExt + Send>(&mut self, n: usize) -> AlignedVec<T, 64>;

    fn generate_random<T: RngExt>(&mut self, n: usize) -> RssShareVec<T>;

    fn generate_random_raw<T: RngExt>(&mut self, n: usize) -> (Vec<T>, Vec<T>);

    fn generate_random_raw_aligned<T: RngExt + Send>(&mut self, n: usize) -> (AlignedVec<T, 64>, AlignedVec<T, 64>);
    ...
    fn send_field_slice<T: NetSerializable>(&self, direction: Direction, elements: &[T]);
...
pub struct MainParty {
    ...
    pub random_local: ChaCha20Rng,
    random_local_threaded: Vec<ChaCha20Rng>,
...
impl MainParty {
    ...
    pub fn setup(
        ...
        let workers = n_worker_threads.unwrap_or(0);
        let random_local_threaded = (0..workers * 2)
            .map(|_| ChaCha20Rng::from_rng(&mut rng).unwrap())
            .collect::<Vec<_>>();
        let party = Self {
            i: party.i,
        ...
        if let Some(prot_str) = prot_string {
            ...
            if prot_str_hash != prot_str_hash_next || prot_str_hash != prot_str_hash_prev {
                let message = format!(
                    "Protocol string does not match the one received from the other parties (hashes don't match). Check that the same cofiguration/protocol/threads etc are used to run all parties. My protocol string: '{}'",
                    prot_str
                );
                println!("{}", message);
                return Err(error::MpcError::Io(io::Error::new(
    ...
    fn split_range_helper(n_parts: usize, end_exclusive: usize) -> Vec<(usize, usize)> {
        let length = if end_exclusive % n_parts == 0 {
            end_exclusive / n_parts
        } else {
            end_exclusive / n_parts + 1
        };
        let total_length = length * n_parts;
        let mut part_lengths = vec![length; n_parts];
        for i in 0..(total_length - end_exclusive) {
            part_lengths[n_parts - 1 - i] -= 1;
        }
        let mut start = 0;
        let mut vec = Vec::with_capacity(n_parts);
        for i in 0..n_parts {
            vec.push((start, start + part_lengths[i]));
            start += part_lengths[i];
        }
        vec
    }

    fn split_range_helper_simple(
        n_parts: usize,
        length: usize,
        end_exclusive: usize,
    ) -> Vec<(usize, usize)> {
    ...
    pub fn split_range_equally(&self, end_exclusive: usize) -> Vec<(usize, usize)> {
        let n_parts = self
            ...
            .map(|tp| tp.current_num_threads())
            .unwrap_or(1);
        if end_exclusive <= n_parts {
            return (0..end_exclusive).map(|i| (i, i + 1)).collect();
        }

        Self::split_range_helper(n_parts, end_exclusive)
    }
    ...
    pub fn split_range_equally_even(&self, end_exclusive: usize) -> Vec<(usize, usize)> {
        ...
        Self::split_range_helper_simple(n_parts, length, end_exclusive)
    }

    fn create_thread_parties_internal<T>(
        &mut self,
        ...
        ranges
            ...
            .map(|((start, end), d)| {
                ...
                let i = self.i;
                ThreadParty::new(i, start, end, random_next, random_prev, random_local, io, d)
            })
            .collect()
    }
    ...
    pub fn create_thread_parties_with_additional_data<T, F: FnMut(usize, usize) -> T>(
        ...
        self.create_thread_parties_internal(
            ranges.clone(),
            ranges.into_iter().map(|(start, end)| data(start, end)),
        )
    }

    pub fn create_thread_parties_with_additiona_data_vec<T>(
        &mut self,
        ranges: Vec<(usize, usize)>,
        data: Vec<T>,
    ) -> Vec<ThreadParty<T>> {
...
#[inline]
fn generate_alpha<T: RngExt + Sub<Output = T>, R: Rng + CryptoRng>(
    next: &mut R,
    prev: &mut R,
    n: usize,
...
impl Party for MainParty {
    ...
    fn generate_random_local_aligned<T: RngExt + Send>(&mut self, n: usize) -> AlignedVec<T, 64> {
        let mut val = vec::from_elem_in(T::ZERO, n, AlignedAllocator::<64>);
        if self.random_local_threaded.is_empty() {
            T::fill(&mut self.random_local, &mut val);
        } else {
            let chunk_size = (val.len() + self.random_local_threaded.len() - 1)
                / self.random_local_threaded.len();
            (
                val.par_chunks_mut(chunk_size),
                &mut self.random_local_threaded,
            )
                .into_par_iter()
                .for_each(|(out, rng)| T::fill(rng, out));
        }
        val
    }
    ...
    fn generate_random_raw<T: RngExt>(&mut self, n: usize) -> (Vec<T>, Vec<T>) {
        (
            T::generate(self.random_prev.as_mut(), n),
            T::generate(self.random_next.as_mut(), n),
        )
    }

    fn generate_random_raw_aligned<T: RngExt + Send>(&mut self, n: usize) -> (AlignedVec<T, 64>, AlignedVec<T, 64>) {
        let mut prev = vec::from_elem_in(T::ZERO, n, AlignedAllocator::<64>);
        let mut next = vec::from_elem_in(T::ZERO, n, AlignedAllocator::<64>);
        rayon::join(
            || T::fill(self.random_prev.as_mut(), &mut prev),
            || T::fill(self.random_next.as_mut(), &mut next)
        );
        (prev, next)
    }
...
pub mod test {
    ...
    use crate::rep3_core::party::correlated_randomness::SharedRng;
    use crate::rep3_core::party::test_export::{
        create_certificates, localhost_setup, simple_localhost_setup,
    };
    ...
    impl NetSerializable for DummyNumber {
        ...
        fn as_byte_vec(
            it: impl IntoIterator<Item = impl std::borrow::Borrow<Self>>,
            _len: usize,
        ) -> Vec<u8> {
    ...
    #[test]
    fn correct_split_range_2() {
        const THREADS: usize = 4;
        fn split_range_test(p: &mut MainParty) {
            let range = p.split_range_equally(5);
            assert_eq!(vec![(0, 2), (2, 3), (3, 4), (4, 5)], range);
        }
        localhost_setup(
            split_range_test,
            split_range_test,
    ...
    fn prot_str_check_correct() {
        ...
        let ((), (), ()) = localhost_connect(
            program(prot_str.clone()),
            program(prot_str.clone()),
            program(prot_str2),
        );
    }
