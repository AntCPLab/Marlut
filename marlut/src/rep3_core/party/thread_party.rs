use std::{borrow::Borrow, cell::OnceCell, ops::Sub, vec};
...
use crate::{
    rep3_core::{
        network::{
            NetSerializable, NetSliceReceiver, NetVectorReceiver,
...
use super::{Party, RngExt, correlated_randomness::SharedRng};
...
impl<T> Party for ThreadParty<T> {
    ...
    fn generate_random_local<E: RngExt>(&mut self, n: usize) -> Vec<E> {
        E::generate(&mut self.random_local, n)
    }

    fn generate_random_local_aligned<E: RngExt + Send>(&mut self, n: usize) -> AlignedVec<E, 64> {
        let mut val = vec::from_elem_in(E::ZERO, n, AlignedAllocator::<64>);
        E::fill(&mut self.random_local, &mut val);
        val
    }
    ...
    fn generate_random_raw<E: RngExt>(&mut self, n: usize) -> (Vec<E>, Vec<E>) {
        (
            E::generate(self.random_prev.as_mut(), n),
            E::generate(self.random_next.as_mut(), n),
        )
    }

    fn generate_random_raw_aligned<E: RngExt + Send>(&mut self, n: usize) -> (AlignedVec<E, 64>, AlignedVec<E, 64>) {
        let mut prev = vec::from_elem_in(E::ZERO, n, AlignedAllocator::<64>);
        let mut next = vec::from_elem_in(E::ZERO, n, AlignedAllocator::<64>);
        rayon::join(
            || E::fill(self.random_prev.as_mut(), &mut prev),
            || E::fill(self.random_next.as_mut(), &mut next)
        );
        (prev, next)
    }
    ...
    fn send_field_slice<N: NetSerializable>(&self, direction: Direction, elements: &[N]) {
        self.io_layer
            .get()
            .unwrap()
...
mod test {
    ...
    use crate::rep3_core::{
        ...
        party::{
            MainParty, Party,
            test::DummyNumber,
            test_export::{PartySetup, TestSetup},
        },
