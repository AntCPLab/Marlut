...
use crate::share::{
    ...
    bs_bool16::BsBool16,
    gf2p64::{
        GF2p64, GF2p64InnerProd, GF2p64Subfield, embed_gf2_deg8, embed_gf2_deg012,
        embed_gf2_deg036, embed_gf4p4_deg2, embed_gf4p4_deg3,
    },
    gf4::BsGF4,
};
use crate::util::aligned_vec::AlignedVec;
...
pub trait MulTripleRecorder<F: Field> {
    ...
    fn create_thread_mul_triple_recorder(
        &self,
        range_start: usize,
        range_end: usize,
    ) -> Self::ThreadMulTripleRecorder;
...
pub trait BitStringMulTripleRecorder {
    ...
    fn record_bit_bitstring_triple(
        &mut self,
        simd_len: usize,
        a_i: &[BsBool16],
        a_ii: &[BsBool16],
...
pub trait Ohv16TripleRecorder {
    /// "Child" recorder type for multi-threading
    type ThreadMulTripleRecorder<'a>: Ohv16TripleRecorder + Sized + Send
    where
        Self: 'a;

    fn record_gf2p9_triple_a(
        &mut self,
        b0i: &[BsBool16],
        b0ii: &[BsBool16],
        b1i: &[BsBool16],
    ...
    /// by threads to record their observed multiplication triples
    /// ranges: Vec of start, end_exclusive of the range that this thread will cover
    fn create_thread_mul_triple_recorders(
        &mut self,
        task_sizes: &[usize],
    ) -> Vec<Self::ThreadMulTripleRecorder<'_>>;
}
...
pub trait MulTripleEncoder {
    ...
    fn add_triples(
        &mut self,
        x: &mut [RssShare<GF2p64>],
        y: &mut [RssShare<GF2p64>],
        zi: &mut GF2p64InnerProd,
        zii: &mut GF2p64InnerProd,
        weight: &mut GF2p64,
        rand: GF2p64,
    );
...
impl<F: Clone> MulTripleVector<F> {
    ...
    fn rss_iter(xi: Vec<F>, xii: Vec<F>) -> impl ExactSizeIterator<Item = RssShare<F>>
    where
        F: Field,
    {
        xi.into_iter()
    ...
    fn drain_rss_iter<'a>(
        xi: &'a mut Vec<F>,
        xii: &'a mut Vec<F>,
    ) -> impl ExactSizeIterator<Item = RssShare<F>> + 'a
    where
        F: Field,
    {
        xi.drain(..)
    ...
    pub fn into_rss_iter(
        self,
    ) -> impl ExactSizeIterator<Item = (RssShare<F>, RssShare<F>, RssShare<F>)>
    where
        F: Field,
    {
        izip!(
            Self::rss_iter(self.ai, self.aii),
            Self::rss_iter(self.bi, self.bii),
            Self::rss_iter(self.ci, self.cii)
        )
    }

    pub fn drain_into_rss_iter<'a>(
        &'a mut self,
    ) -> impl ExactSizeIterator<Item = (RssShare<F>, RssShare<F>, RssShare<F>)> + 'a
    where
        F: Field,
    {
        izip!(
            Self::drain_rss_iter(&mut self.ai, &mut self.aii),
            Self::drain_rss_iter(&mut self.bi, &mut self.bii),
            Self::drain_rss_iter(&mut self.ci, &mut self.cii)
        )
    }

    pub fn as_mut_slices(
        &mut self,
    ) -> (&mut [F], &mut [F], &mut [F], &mut [F], &mut [F], &mut [F]) {
        (
            &mut self.ai,
            &mut self.aii,
            &mut self.bi,
...
pub struct InnerProductTriple<F> {
    pub ai: AlignedVec<F, 64>,
    pub aii: AlignedVec<F, 64>,
    pub bi: AlignedVec<F, 64>,
    pub bii: AlignedVec<F, 64>,
    pub ci: AlignedVec<F, 64>,
    pub cii: AlignedVec<F, 64>,
    pub ri: AlignedVec<F, 64>,
    pub rii: AlignedVec<F, 64>,
    pub elems_per_block: usize,
    pub elems_per_lookup: usize,
}

pub struct InnerProductTripleBoolean<F> {
    pub ai: Vec<F>,
    pub aii: Vec<F>,
    pub bi: Vec<F>,
    pub bii: Vec<F>,
    ...
    pub bits_per_lookup: usize,
    pub shifts: Vec<usize>,
}

pub struct ManyToOneMulTriple<F> {
    pub ai: AlignedVec<F, 64>,
    pub aii: AlignedVec<F, 64>,
    pub bi: AlignedVec<F, 64>,
    pub bii: AlignedVec<F, 64>,
...
pub struct ManyToOneMulTripleBoolean<F> {
    pub ai: Vec<F>,
    pub aii: Vec<F>,
    pub bi: Vec<F>,
    pub bii: Vec<F>,
...
impl<T: Copy + Debug + HasZero, const N: usize> GenericMulTripleVector<T, N> {
    ...
    pub fn create_slice(&mut self, task_sizes: &[usize]) -> Vec<GenericMulTripleVectorSlice<T, N>> {
        ...
        let len = self.len();
        let mut slice: Vec<_> = self
            .0
            .iter_mut()
            .map(|sl| &mut sl[len - total_range..])
            .collect();
        let mut res = Vec::new();
        for len in task_sizes {
            let (front, back): (Vec<_>, Vec<_>) =
                slice.into_iter().map(|sl| sl.split_at_mut(*len)).unzip();
            slice = back;
...
fn zip_eq9<T: Copy, const N: usize>(
    v: &[Vec<T>; N],
    start: usize,
) -> impl Iterator<Item = [T; 9]> + '_ {
    izip!(
        v[start].iter().copied(),
        v[start + 1].iter().copied(),
        v[start + 2].iter().copied(),
        v[start + 3].iter().copied(),
        v[start + 4].iter().copied(),
        v[start + 5].iter().copied(),
        v[start + 6].iter().copied(),
        v[start + 7].iter().copied(),
        v[start + 8].iter().copied()
    )
    .map(|t| t.into())
}

fn zip_eq9_slice<T: Copy, const N: usize>(
    v: [&[T]; N],
    start: usize,
) -> impl Iterator<Item = [T; 9]> + '_ {
    izip!(
...
fn par_zip_eq9<T: Copy + Sync>(
    v: &GenericMulTripleVector<T, 18>,
    chunk_size: usize,
    start: usize,
) -> impl IndexedParallelIterator<Item = [&[T]; 9]> + '_ {
    v.0[start]
        .par_chunks(chunk_size)
        .zip_eq(v.0[start + 1].par_chunks(chunk_size))
        ...
        .map(|((((((((t0, t1), t2), t3), t4), t5), t6), t7), t8)| {
...
impl Ohv16TripleVector {
    pub fn new() -> Self {
        Self {
            a: GenericMulTripleVector::new(),
            b: GenericMulTripleVector::new(),
            c: GenericMulTripleVector::new(),
        }
    }
...
impl Ohv16TripleRecorder for Ohv16TripleVector {
    ...
    fn create_thread_mul_triple_recorders(
        ...
        izip!(
            self.a.create_slice(task_sizes),
            self.b.create_slice(task_sizes),
            self.c.create_slice(task_sizes)
        )
        .map(|(a, b, c)| Ohv16TripleVectorSlice { a, b, c })
        .collect()
    }
}

impl<'a> Ohv16TripleRecorder for Ohv16TripleVectorSlice<'a> {
    type ThreadMulTripleRecorder<'b>
        = Ohv16TripleVectorSlice<'b>
    where
        Self: 'b;
    fn record_gf2p9_triple_a(
        &mut self,
    ...
    fn record_gf2p9_triple_c(&mut self, bi: [&[BsBool16]; 9], bii: [&[BsBool16]; 9]) {
        ...
        self.c.extend_from_slice([
            b0i, b1i, b2i, b3i, b4i, b5i, b6i, b7i, b8i, b0ii, b1ii, b2ii, b3ii, b4ii, b5ii, b6ii,
            b7ii, b8ii,
        ]);
    }
    fn create_thread_mul_triple_recorders(
...
impl<'a> MulTripleEncoder for Ohv16TripleEncoder<'a> {
    ...
    fn add_triples(
        ...
        .for_each(|(dst, a0i, a1i, a2i, a0ii, a1ii, a2ii)| {
        ...
        .for_each(|(dst, b0i, b1i, b2i, b0ii, b1ii, b2ii)| {
        ...
        izip!(
            x.chunks_exact_mut(16),
            zip_eq9(&self.0.c.0, 0),
            zip_eq9(&self.0.c.0, 9)
        )
        .for_each(|(x_chunks, ci, cii)| {
            embed_gf2_deg8(
                &mut si, ci[0], ci[1], ci[2], ci[3], ci[4], ci[5], ci[6], ci[7], ci[8],
            );
            embed_gf2_deg8(
                &mut sii, cii[0], cii[1], cii[2], cii[3], cii[4], cii[5], cii[6], cii[7], cii[8],
            );
            for i in 0..16 {
    ...
    fn add_triples_par(
        ...
        x.par_chunks_mut(16 * chunk_size)
            .zip_eq(self.0.a[0].par_chunks(chunk_size))
            ...
            .for_each(|((((((x, a0i), a1i), a2i), a0ii), a1ii), a2ii)| {
                let mut si = [GF2p64::ZERO; 16];
                let mut sii = [GF2p64::ZERO; 16];
                izip!(x.chunks_exact_mut(16), a0i, a1i, a2i, a0ii, a1ii, a2ii).for_each(
                    |(dst, a0i, a1i, a2i, a0ii, a1ii, a2ii)| {
                        embed_gf2_deg012(&mut si, *a0i, *a1i, *a2i);
                        embed_gf2_deg012(&mut sii, *a0ii, *a1ii, *a2ii);
                        for i in 0..16 {
                            dst[i].si = si[i];
        ...
        y.par_chunks_mut(16 * chunk_size)
            .zip_eq(self.0.b[0].par_chunks(chunk_size))
            ...
            .for_each(|((((((y, b0i), b1i), b2i), b0ii), b1ii), b2ii)| {
                let mut si = [GF2p64::ZERO; 16];
                let mut sii = [GF2p64::ZERO; 16];
                izip!(y.chunks_exact_mut(16), b0i, b1i, b2i, b0ii, b1ii, b2ii).for_each(
                    |(dst, b0i, b1i, b2i, b0ii, b1ii, b2ii)| {
                        embed_gf2_deg036(&mut si, *b0i, *b1i, *b2i);
                        embed_gf2_deg036(&mut sii, *b0ii, *b1ii, *b2ii);
                        for i in 0..16 {
                            dst[i].si = si[i];
        ...
        let z_vec: Vec<_> = x
            .par_chunks_mut(16 * chunk_size)
            .zip_eq(par_zip_eq9(&self.0.c, chunk_size, 0))
            ...
            .map(|(((x, ci), cii), rand)| {
                ...
                .for_each(|(x_chunks, ci, cii)| {
                    ...
                    embed_gf2_deg8(
                        &mut sii, cii[0], cii[1], cii[2], cii[3], cii[4], cii[5], cii[6], cii[7],
                        cii[8],
                    );
                    for i in 0..16 {
                        x_chunks[i] = x_chunks[i] * local_weight;
...
impl BitStringMulTripleRecorder for MulTripleVector<GF2p64> {
    fn record_bit_bitstring_triple(
        ...
        while from < to {
            ...
            GF2p64::extend_from_bitstring(
                &mut self.bi,
                &b_i[simd_len * from..simd_len * to],
                simd_len,
            );
            GF2p64::extend_from_bitstring(
                &mut self.bii,
                &b_ii[simd_len * from..simd_len * to],
                simd_len,
            );
            GF2p64::extend_from_bitstring(
                &mut self.ci,
                &c_i[simd_len * from..simd_len * to],
                simd_len,
            );
            GF2p64::extend_from_bitstring(
                &mut self.cii,
                &c_ii[simd_len * from..simd_len * to],
                simd_len,
            );
...
macro_rules! mul_triple_encoder_impl {
    ($encode_name:ident, $yield_size:literal) => {
        ...
        fn add_triples(
            ...
            izip!(
                x.chunks_exact_mut($yield_size),
                y.chunks_exact_mut($yield_size),
                &self.0.ai,
                &self.0.aii,
                &self.0.bi,
                &self.0.bii,
        ...
        fn add_triples_par(
            ...
            let zvec: Vec<_> = x
                .par_chunks_mut(chunk_size * $yield_size)
...
#[inline]
fn encode_gf2p64(
    dst_a: &mut [RssShare<GF2p64>],
    dst_b: &mut [RssShare<GF2p64>],
    dst_c: &mut [RssShare<GF2p64>; 1],
    ai: GF2p64,
    aii: GF2p64,
    bi: GF2p64,
    bii: GF2p64,
...
#[inline]
fn encode_gf2p64_subfield<F: GF2p64Subfield>(
    dst_a: &mut [RssShare<GF2p64>],
    dst_b: &mut [RssShare<GF2p64>],
    dst_c: &mut [RssShare<GF2p64>; 1],
    ai: F,
...
impl GF4p4TripleVector {
    ...
    pub fn create_thread_mul_triple_recorders(
        &mut self,
        task_sizes: &[usize],
    ) -> Vec<GF4P4TripleVectorChild> {
        let total_range = task_sizes.iter().sum();
        // get a slice of total_range elements
        self.a.append(&mut vec![
            RssShare::from(GF2p64::ZERO, GF2p64::ZERO);
            total_range
        ]);
...
impl GF4p4TripleRecorder for GF4p4TripleVector {
    ...
    fn record_mul_triple(
        ...
        self.a.push(RssShare::from(
            embed_gf4p4_deg2(x0_i_l, x1_i_l),
            embed_gf4p4_deg2(x0_ii_l, x1_ii_l),
        ));
        self.a.push(RssShare::from(
            embed_gf4p4_deg2(x0_i_h, x1_i_h),
            embed_gf4p4_deg2(x0_ii_h, x1_ii_h),
        ));
        ...
        self.b.push(RssShare::from(
            embed_gf4p4_deg2(y0_i_l, y1_i_l),
            embed_gf4p4_deg2(y0_ii_l, y1_ii_l),
        ));
        self.b.push(RssShare::from(
            embed_gf4p4_deg2(y0_i_h, y1_i_h),
            embed_gf4p4_deg2(y0_ii_h, y1_ii_h),
        ));
        ...
        self.c.push(RssShare::from(
            embed_gf4p4_deg3(z0_i_l, z1_i_l, z2_i_l),
            embed_gf4p4_deg3(z0_ii_l, z1_ii_l, z2_ii_l),
        ));
        self.c.push(RssShare::from(
            embed_gf4p4_deg3(z0_i_h, z1_i_h, z2_i_h),
            embed_gf4p4_deg3(z0_ii_h, z1_ii_h, z2_ii_h),
        ));
    }
}
...
impl<'a> GF4p4TripleRecorder for GF4P4TripleVectorChild<'a> {
    ...
    fn record_mul_triple(
        ...
        let (x1_i_h, x1_i_l) = x1_i.unpack();
        let (x1_ii_h, x1_ii_l) = x1_ii.unpack();
        self.a[self.idx] = RssShare::from(
            embed_gf4p4_deg2(x0_i_l, x1_i_l),
            embed_gf4p4_deg2(x0_ii_l, x1_ii_l),
        );
        self.a[self.idx + 1] = RssShare::from(
            embed_gf4p4_deg2(x0_i_h, x1_i_h),
            embed_gf4p4_deg2(x0_ii_h, x1_ii_h),
        );
        ...
        let (y1_i_h, y1_i_l) = y1_i.unpack();
        let (y1_ii_h, y1_ii_l) = y1_ii.unpack();
        self.b[self.idx] = RssShare::from(
            embed_gf4p4_deg2(y0_i_l, y1_i_l),
            embed_gf4p4_deg2(y0_ii_l, y1_ii_l),
        );
        self.b[self.idx + 1] = RssShare::from(
            embed_gf4p4_deg2(y0_i_h, y1_i_h),
            embed_gf4p4_deg2(y0_ii_h, y1_ii_h),
        );
        ...
        let (z2_i_h, z2_i_l) = z2_i.unpack();
        let (z2_ii_h, z2_ii_l) = z2_ii.unpack();
        self.c[self.idx] = RssShare::from(
            embed_gf4p4_deg3(z0_i_l, z1_i_l, z2_i_l),
            embed_gf4p4_deg3(z0_ii_l, z1_ii_l, z2_ii_l),
        );
        self.c[self.idx + 1] = RssShare::from(
            embed_gf4p4_deg3(z0_i_h, z1_i_h, z2_i_h),
            embed_gf4p4_deg3(z0_ii_h, z1_ii_h, z2_ii_h),
        );
        self.idx += 2;
...
impl<'a> MulTripleEncoder for GF4p4TripleEncoder<'a> {
    ...
    fn add_triples_par(
        ...
        x.copy_from_slice(&self.0.a);
        y.copy_from_slice(&self.0.b);
        let zvec: Vec<RssShare<GF2p64>> = x
...
pub mod test {
    ...
    use crate::rep3_core::{
        party::{MainParty, RngExt},
        share::{HasZero, RssShare},
        test::localhost_setup,
    };
    ...
    use crate::{
        share::{
            Field,
            bs_bool16::BsBool16,
            gf2p64::{GF2p64, GF2p64InnerProd},
            gf4::BsGF4,
            gf8::GF8,
            test::{consistent, secret_share, secret_share_vector},
        },
        util::mul_triple_vec::{
            BitStringMulTripleRecorder, BsBool16Encoder, BsGF4Encoder, GF2p64Encoder,
            GF2p64SubfieldEncoder, GF4p4TripleRecorder, GF4p4TripleVector, MulTripleRecorder,
    ...
    fn record_bit_bitstring_triple_simple() {
        ...
        // add a single bit
        rec.record_bit_bitstring_triple(
            1,
            &[BsBool16::new(1)],
            &[BsBool16::new(0)],
            &[BsBool16::new(1)],
    ...
    fn record_bit_bitstring_triple_simd() {
        ...
        rec.record_bit_bitstring_triple(
            2,
            &[BsBool16::new(1), BsBool16::new(1)],
            &[BsBool16::new(0), BsBool16::new(1)],
            &[BsBool16::new(1), BsBool16::new(0)],
            &[BsBool16::new(0), BsBool16::new(1)],
            &[BsBool16::new(1), BsBool16::new(0)],
            &[BsBool16::new(1), BsBool16::new(0)],
        );
        ...
        let bs_65 = (0..65).map(|_| BsBool16::new(1)).collect_vec();
        let bs_65_half = (0..32)
            .map(|_| BsBool16::new(0))
            .chain((0..33).map(|_| BsBool16::new(1)));
        let input = bs_65
            .into_iter()
            .zip(bs_65_half)
            .flat_map(|(simd1, simd2)| [simd1, simd2])
            .collect_vec();
        rec.record_bit_bitstring_triple(
            2,
            &[BsBool16::new(1), BsBool16::new(1)],
    ...
    pub fn check_correct_encoding<Enc: MulTripleEncoder>(
        mut enc1: Enc,
        mut enc2: Enc,
        mut enc3: Enc,
        n_expected_triples: usize,
    ) {
        ...
        let mut rng = thread_rng();
        let (weight, rand) = GF2p64::generate(&mut rng, 2)
            .into_iter()
            .collect_tuple()
            .unwrap();
    ...
    pub fn check_correct_encoding_par<Enc: MulTripleEncoder + Send>(
        enc1: Enc,
        enc2: Enc,
        enc3: Enc,
        n_expected_triples: usize,
        ...
        let (((x1, y1, z1), _), ((x2, y2, z2), _), ((x3, y3, z3), _)) = localhost_setup(
            program(enc1, weight),
            program(enc2, weight),
            program(enc3, weight),
            Some(3),
    ...
    fn generate_and_fill_random_triples<F: Field, R: MulTripleRecorder<F>>(
        rec1: &mut R,
        rec2: &mut R,
        rec3: &mut R,
        n: usize,
    ...
    fn record_mul_triple<F: Field, R: MulTripleRecorder<F>>(
        rec: &mut R,
        x: Vec<RssShare<F>>,
        y: Vec<RssShare<F>>,
        z: Vec<RssShare<F>>,
    ...
    fn gf2p64_encoder_correct() {
        ...
        check_correct_encoding(
            GF2p64Encoder(&mut rec1),
            GF2p64Encoder(&mut rec2),
            GF2p64Encoder(&mut rec3),
            N,
        );

        // now test if the encoding is correct when encoding in parallel
        check_correct_encoding_par(
            GF2p64Encoder(&mut rec1),
            GF2p64Encoder(&mut rec2),
            GF2p64Encoder(&mut rec3),
            N,
    ...
    fn gf2p64_subfield_encoder_gf8_correct() {
        ...
        // now test if the encoding is correct
        check_correct_encoding(
            GF2p64SubfieldEncoder(&mut rec1),
            GF2p64SubfieldEncoder(&mut rec2),
            GF2p64SubfieldEncoder(&mut rec3),
            N,
        );
    ...
    fn bsbool16_encoder_correct() {
        ...
        // now test if the encoding is correct
        check_correct_encoding(
            BsBool16Encoder(&mut rec1),
            BsBool16Encoder(&mut rec2),
            BsBool16Encoder(&mut rec3),
            16 * N,
        );
    ...
    fn bs_gf4_encoder_correct() {
        ...
        // now test if the encoding is correct
        check_correct_encoding(
            BsGF4Encoder(&mut rec1),
            BsGF4Encoder(&mut rec2),
            BsGF4Encoder(&mut rec3),
            2 * N,
        );
    ...
    fn ohv16_triple_vector_encoder_correct() {
        ...
        izip!(r0, r1, r2, r3, r4, r5).for_each(|(r0, r1, r2, r3, r4, r5)| {
            ...
            fn record_gf2p9_triple_c(
                rec: &mut Ohv16TripleVector,
                expected_i: Vec<BsBool16>,
                expected_ii: Vec<BsBool16>,
            ) {
                let expected_i: [_; 9] = expected_i
                    .iter()
                    .map(|x| slice::from_ref(x))
                    .collect_vec()
                    .try_into()
                    .unwrap();
                let expected_ii: [_; 9] = expected_ii
                    .iter()
                    .map(|x| slice::from_ref(x))
                    .collect_vec()
                    .try_into()
        ...
        // now test if the encoding is correct
        check_correct_encoding(
            Ohv16TripleEncoder(&mut rec1),
            Ohv16TripleEncoder(&mut rec2),
            Ohv16TripleEncoder(&mut rec3),
            16 * N,
        );
