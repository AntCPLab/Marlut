...
use crate::{
    ...
    util::mul_triple_vec::{GF4p4TripleRecorder, GF4p4TripleVector, MulTripleRecorder},
...
pub fn gf8_inv_layer<P: Party, Rec: MulTripleRecorder<BsGF4>>(
    party: &mut P,
    triple_rec: &mut Rec,
    si: &mut [GF8],
    sii: &mut [GF8],
    ...
    let a_h_plus_a_l_i = ah_i
        .into_iter()
        .zip(al_i)
        .map(|(ah, al)| ah + al)
        .collect_vec();
    let a_h_plus_a_l_ii = ah_ii
        .into_iter()
        .zip(al_ii)
        .map(|(ah, al)| ah + al)
        .collect_vec();
...
pub fn gf8_inv_layer_mt<Rec: MulTripleRecorder<BsGF4>>(
    party: &mut MainParty,
    triple_rec: &mut Rec,
    si: &mut [GF8],
    sii: &mut [GF8],
    ...
    let observed_triples = party.run_in_threadpool(|| {
        threads
            ...
            .map(|(((mut thread_party, si), sii), prep_ohv)| {
                ...
                gf8_inv_layer(&mut thread_party, &mut triple_rec, si, sii, prep_ohv)
                    .map(|()| triple_rec)
            })
...
///
/// The output, the share `[[x^-1]]_i`, is written into `(s_i,s_ii)`.
pub fn gf8_inv_layer_gf4p4_check<P: Party, Rec: GF4p4TripleRecorder>(
    party: &mut P,
    triple_rec: &mut Rec,
    si: &mut [GF8],
    sii: &mut [GF8],
    ...
    .for_each(
        |(
            ...
            triple_rec.record_mul_triple(
                ...
                ah_times_al_i,
                ah_times_al_ii,
                *ah_prime_i + ali.square(),
                *ah_prime_ii + alii.square(),
                *al_prime_i + *ah_prime_i,
                *al_prime_ii + *ah_prime_ii,
            );
        },
    );
...
pub fn gf8_inv_layer_gf4p4_check_mt(
    party: &mut MainParty,
    triple_rec: &mut GF4p4TripleVector,
    si: &mut [GF8],
    sii: &mut [GF8],
    rnd_ohv: &[RndOhv16Output],
) -> MpcResult<()> {
    ...
    threads
        .iter_mut()
        .zip_eq(recorders)
        .for_each(|(tp, rec)| tp.additional_data = Some(rec));
...
fn ss_to_rss_layer<P: Party>(
    ...
    party.send_field::<BsGF4>(Direction::Previous, a_i.iter().chain(b_i.iter()), 2 * n);
    let res = rcv_ii.rcv()?;
...
mod test {
    use crate::{
        aes::test::{
            ...
            test_aes256_keyschedule_gf8, test_aes256_no_keyschedule_gf8,
            test_inv_aes128_no_keyschedule_gf8, test_sub_bytes,
        },
        wollut16_malsec::test::{WL16ASSetup, WL16BitString, WL16DefaultParams, WL16OhvCheck},
    };
    ...
    fn inv_aes128_no_keyschedule_mt() {
        const N_THREADS: usize = 3;
        test_inv_aes128_no_keyschedule_gf8::<WL16ASSetup<WL16DefaultParams>, _>(
            100,
            Some(N_THREADS),
        )
    }
