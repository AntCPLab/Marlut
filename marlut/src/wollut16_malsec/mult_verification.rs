...
use crate::{
    share::{
        Field, HasTwo, InnerProduct, Invertible,
...
pub fn verify_multiplication_triples(
    ...
    triples: &mut [&mut (dyn MulTripleEncoder + Send + Sync)],
    dont_clear: bool,
) -> MpcResult<bool> {
...
fn reconstruct<F: Field + DigestExt>(
    party: &mut MainParty,
    context: &mut BroadcastContext,
    rho: RssShare<F>,
) -> MpcResult<F> {
...
mod test {
    ...
    use crate::{
        ...
        util::mul_triple_vec::{
            BsBool16Encoder, BsGF4Encoder, GF2p64Encoder, GF2p64SubfieldEncoder, MulTripleRecorder,
            MulTripleVector,
        },
        wollut16_malsec::{
            WL16ASParty,
            mult_verification::{
                verify_dot_product_opt, verify_multiplication_triples,
                verify_multiplication_triples_mt,
            },
            test::{WL16DefaultParams, localhost_setup_wl16as},
        },
    };
    use itertools::izip;
    ...
    fn test_lagrange() {
        ...
        let ((r1, _), (r2, _), (r3, _)) =
            localhost_setup_wl16as::<WL16DefaultParams, _, _, _, _, _, _>(
    ...
    fn test_inner_prod_soundness() {
        ...
        let program = |a: Vec<RssShare<GF2p64>>, b: Vec<RssShare<GF2p64>>, c: RssShare<GF2p64>| {
            move |p: &mut MainParty| {
                verify_dot_product(p, &mut BroadcastContext::new(), a, b, c).unwrap()
            }
        };
        let ((r1, _), (r2, _), (r3, _)) = PartySetup::localhost_setup(
            program(a1, b1, c1),
        ...
        let program =
            |a: Vec<RssShare<BsGF4>>, b: Vec<RssShare<BsGF4>>, c: Vec<RssShare<BsGF4>>| {
                move |p: &mut MainParty| {
                    ...
                    let res = verify_multiplication_triples(
                        p,
                        &mut context,
                        &mut [&mut BsGF4Encoder(&mut triples)],
                        false,
                    )
                    .unwrap();
                    p.compare_view(context).unwrap();
            ...
            |a: Vec<RssShare<BsBool16>>, b: Vec<RssShare<BsBool16>>, c: Vec<RssShare<BsBool16>>| {
                move |p: &mut MainParty| {
                    ...
                    let res = verify_multiplication_triples(
                        p,
                        &mut context,
                        &mut [&mut BsBool16Encoder(&mut triples)],
                        false,
                    )
                    .unwrap();
                    p.compare_view(context).unwrap();
            ...
            |a: Vec<RssShare<GF2p64>>, b: Vec<RssShare<GF2p64>>, c: Vec<RssShare<GF2p64>>| {
                move |p: &mut MainParty| {
                    ...
                    let res = verify_multiplication_triples(
                        p,
                        &mut context,
                        &mut [&mut GF2p64Encoder(&mut triples)],
                        false,
                    )
                    .unwrap();
                    p.compare_view(context).unwrap();
        ...
        let program =
            |a: Vec<RssShare<BsGF4>>, b: Vec<RssShare<BsGF4>>, c: Vec<RssShare<BsGF4>>| {
                move |p: &mut MainParty| {
                    ...
                    verify_multiplication_triples(
                        p,
                        &mut BroadcastContext::new(),
                        &mut [&mut BsGF4Encoder(&mut triples)],
                        false,
                    )
                    .unwrap()
    ...
    fn test_gf2_and_gf4_and_gf64_mul_verify_correctness() {
        ...
        let program = |a: Vec<RssShare<BsGF4>>,
                       b: Vec<RssShare<BsGF4>>,
                       c: Vec<RssShare<BsGF4>>,
                       x: Vec<RssShare<BsBool16>>,
                       y: Vec<RssShare<BsBool16>>,
                       z: Vec<RssShare<BsBool16>>,
                       u: Vec<RssShare<GF2p64>>,
                       v: Vec<RssShare<GF2p64>>,
                       w: Vec<RssShare<GF2p64>>| {
            move |p: &mut MainParty| {
                let mut context = BroadcastContext::new();
                ...
                izip!(u, v, w).for_each(|(u, v, w)| {
                    gf64_triples.record_mul_triple(
                        &[u.si],
                        &[u.sii],
                        &[v.si],
                        &[v.sii],
                ...
                let res = verify_multiplication_triples(
                    ...
                    &mut context,
                    &mut [
                        &mut BsGF4Encoder(&mut gf4_triples),
                        &mut BsBool16Encoder(&mut gf2_triples),
                        &mut GF2p64Encoder(&mut gf64_triples),
                    ],
                    false,
                )
                .unwrap();
    ...
    fn test_gf2_and_gf4_and_gf64_mul_verify_correctness_mt() {
        ...
        let program = |a: Vec<RssShare<BsGF4>>,
            ...
            move |p: &mut MainParty| {
                ...
                let res = verify_multiplication_triples_mt(
                    p,
                    &mut context,
                    &mut [
                        &mut BsGF4Encoder(&mut gf4_triples),
        ...
        let program = |a: Vec<RssShare<GF8>>, b: Vec<RssShare<GF8>>, c: Vec<RssShare<GF8>>| {
            move |p: &mut MainParty| {
                ...
                let res = verify_multiplication_triples(
                    p,
                    &mut context,
                    &mut [&mut GF2p64SubfieldEncoder(&mut triples)],
                    false,
                )
                .unwrap();
                p.compare_view(context).unwrap();
