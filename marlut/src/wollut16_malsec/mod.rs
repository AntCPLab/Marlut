...
use crate::{
    ...
    share::{bs_bool16::BsBool16, gf2p64::GF2p64, gf4::BsGF4, gf8::GF8},
    util::{
        ArithmeticBlackBox,
        mul_triple_vec::{
            BsBool16Encoder, BsGF4Encoder, GF2p64Encoder, GF4p4TripleEncoder, GF4p4TripleVector,
            MulTripleRecorder, MulTripleVector, Ohv16TripleEncoder, Ohv16TripleVector,
        },
    },
    ...
    fn prepare_rand_ohv(&mut self, n: usize) -> MpcResult<()> {
        let mut new = if self.inner.has_multi_threading() && self.inner.num_worker_threads() <= n {
            match &self.prep_check {
                ...
                PrepCheckType::OhvCheck => wollut16::offline::generate_ohv16_opt_check_mt(
                    &mut self.inner,
                    &mut self.ohv_triples_to_check,
                    n,
                )?,
            }
        } else {
            match &self.prep_check {
                ...
                PrepCheckType::OhvCheck => wollut16::offline::generate_ohv16_opt_check(
                    &mut self.inner,
                    &mut self.ohv_triples_to_check,
                    n,
                )?,
        ...
        let res = if self.inner.has_multi_threading() {
            mult_verification::verify_multiplication_triples_mt(
                ...
                &mut [
                    ...
                    &mut GF2p64Encoder(&mut self.gf64_triples_to_check),
                    &mut Ohv16TripleEncoder(&mut self.ohv_triples_to_check),
...
impl ArithmeticBlackBox<GF8> for WL16ASParty {
    ...
    fn input_round(
        &mut self,
        _my_input: &[GF8],
    ) -> MpcResult<(Vec<RssShare<GF8>>, Vec<RssShare<GF8>>, Vec<RssShare<GF8>>)> {
        unimplemented!()
    }
...
impl GF8InvBlackBox for WL16ASParty {
    ...
    fn do_preprocessing(
        ...
        match &self.prep_check {
            PrepCheckType::Simple => self
                .gf2_triples_to_check
                .reserve_for_more_triples(10 * n_blocks + div16_ceil(n_rnd_ohv_ks)),
            PrepCheckType::BitString => self
                .gf64_triples_to_check
                .reserve_for_more_triples(div16_ceil(n_rnd_ohv_ks + n_rnd_ohv) * 16 * 3),
            PrepCheckType::OhvCheck => self
                .ohv_triples_to_check
                .reserve_for_more_triples(div16_ceil(n_rnd_ohv + n_rnd_ohv_ks)),
        }
        ...
        if self.use_gf4p4_check {
            ...
            self.gf4p4_triples_to_check
                .reserve_for_more_triples(n_mul_ks + n_mul);
        } else {
    ...
    fn gf8_inv(&mut self, si: &mut [GF8], sii: &mut [GF8]) -> MpcResult<()> {
        ...
        if self.prep_ohv.len() < si.len() {
            panic!(
                "Not enough pre-processed random one-hot vectors available. Use WL16ASParty::prepare_rand_ohv to generate them."
            );
        }
        let remainning = self.prep_ohv.len() - si.len();
        if self.inner.has_multi_threading() && self.inner.num_worker_threads() <= si.len() {
            if self.use_gf4p4_check {
                online::gf8_inv_layer_gf4p4_check_mt(
                    &mut self.inner,
                    &mut self.gf4p4_triples_to_check,
                    si,
                    sii,
                    &self.prep_ohv[remainning..],
                )?
            } else {
                online::gf8_inv_layer_mt(
                    &mut self.inner,
            ...
            if self.use_gf4p4_check {
                online::gf8_inv_layer_gf4p4_check(
                    &mut self.inner,
                    &mut self.gf4p4_triples_to_check,
                    si,
                    sii,
...
mod test {
    ...
    pub fn localhost_setup_wl16as<
        P: WL16Params,
        T1: Send,
        F1: Send + FnOnce(&mut WL16ASParty) -> T1,
        T2: Send,
        F2: Send + FnOnce(&mut WL16ASParty) -> T2,
        T3: Send,
        F3: Send + FnOnce(&mut WL16ASParty) -> T3,
    >(
        f1: F1,
        f2: F2,
        f3: F3,
        n_worker_threads: Option<usize>,
    ) -> ((T1, WL16ASParty), (T2, WL16ASParty), (T3, WL16ASParty)) {
        fn adapter<P: WL16Params, T, Fx: FnOnce(&mut WL16ASParty) -> T>(
            conn: ConnectedParty,
            f: Fx,
            n_worker_threads: Option<usize>,
        ) -> (T, WL16ASParty) {
            let mut party = WL16ASParty::setup(
                conn,
                P::CHECK_AFTER_PREP,
                P::CHECK_AFTER_SBOX,
                P::PREP_CHECK,
        ...
        localhost_connect(
            move |conn_party| adapter::<P, _, _>(conn_party, f1, n_worker_threads),
            move |conn_party| adapter::<P, _, _>(conn_party, f2, n_worker_threads),
            move |conn_party| adapter::<P, _, _>(conn_party, f3, n_worker_threads),
        )
    }
