...
use crate::{
    chida, lut256,
    share::{Field, bs_bool16::BsBool16, gf4::GF4},
    util::mul_triple_vec::{BitStringMulTripleRecorder, MulTripleRecorder, Ohv16TripleRecorder},
    wollut16::{RndOhv16, RndOhv16Output},
};
...
pub fn generate_random_ohv16<P: Party, Rec: MulTripleRecorder<BsBool16>>(
    party: &mut P,
    triple_rec: &mut Rec,
    n: usize,
...
pub fn generate_random_ohv16_mt<'a, Rec: MulTripleRecorder<BsBool16>>(
    ...
    let threads = party.create_thread_parties_with_additional_data(ranges, |start, end| {
        Some(triple_rec.create_thread_mul_triple_recorder(start, end))
    });
    ...
    party.run_in_threadpool(|| {
        threads
            ...
            .map(|mut thread_party| {
                ...
                let rnd_ohv =
                    generate_ohv16(&mut thread_party, &mut rec, task_n * 16, r0, r1, r2, r3)
                        .unwrap();
                (rnd_ohv, rec)
            })
...
#[inline]
pub fn generate_ohv16_bitslice<P: Party, Rec: MulTripleRecorder<BsBool16>>(
    party: &mut P,
    triple_rec: &mut Rec,
...
) -> MpcResult<[Vec<RssShare<BsBool16>>; 16]> {
    let n16 = r0.len();
...
fn generate_ohv16<P: Party, Rec: MulTripleRecorder<BsBool16>>(
    party: &mut P,
    triple_rec: &mut Rec,
    n: usize,
    r0: Vec<RssShare<BsBool16>>,
...
fn ohv16_unbitslice_and_pack(
    sliced_ohv_bits: [Vec<RssShare<BsBool16>>; 16],
    r0: Vec<RssShare<BsBool16>>,
    r1: Vec<RssShare<BsBool16>>,
    r2: Vec<RssShare<BsBool16>>,
    r3: Vec<RssShare<BsBool16>>,
...
pub fn generate_random_ohv16_bitstring<P: Party, Rec: BitStringMulTripleRecorder>(
    party: &mut P,
    triple_rec: &mut Rec,
    n: usize,
) -> MpcResult<Vec<RndOhv16Output>> {
...
pub fn generate_random_ohv16_bitstring_mt<
    'a,
    Rec: BitStringMulTripleRecorder + MulTripleRecorder<F>,
    F: Field,
>(
    party: &'a mut MainParty,
    triple_rec: &mut Rec,
    ...
    party.run_in_threadpool(|| {
        threads
            ...
            .map(|mut thread_party| {
                ...
                let mut rec = thread_party.additional_data.take().unwrap();
                let rnd_ohv =
                    generate_random_ohv16_bitstring(&mut thread_party, &mut rec, task_n * 16)
                        .unwrap();
                (rnd_ohv, rec)
            })
            .collect_into_vec(&mut rnd_ohv);
...
pub fn generate_ohv16_opt_check<P: Party, Rec: Ohv16TripleRecorder>(
    party: &mut P,
    triple_rec: &mut Rec,
    n: usize,
) -> MpcResult<Vec<RndOhv16Output>> {
...
pub fn generate_ohv16_opt_check_mt<Rec: Ohv16TripleRecorder>(
    party: &mut MainParty,
    triple_rec: &mut Rec,
    n: usize,
) -> MpcResult<Vec<RndOhv16Output>> {
    ...
    let task_sizes = ranges
        .iter()
        .map(|(start, end)| 2 * (end - start))
        .collect_vec();
    let slices = triple_rec
    ...
    party.run_in_threadpool(|| {
        thread_parties
            ...
            .map(|mut thread_party| {
                ...
                let mut rec = thread_party.additional_data.take().unwrap();
                generate_ohv16_opt_check(&mut thread_party, &mut rec, 16 * n_blocks_per_thread)
                    .unwrap()
            })
...
// implements Protocol 7 with fixed inputs
#[inline]
pub fn generate_ohv16_bitslice_opt_check<P: Party, Rec: Ohv16TripleRecorder>(
    party: &mut P,
    triple_rec: &mut Rec,
    r0: &Vec<RssShare<BsBool16>>,
    r1: &Vec<RssShare<BsBool16>>,
    ...
    triple_rec.record_gf2p9_triple_a(
        &ai[..n16],
        &aii[..n16],
        &ai[3 * n16..4 * n16],
        &aii[3 * n16..4 * n16],
    ...
    triple_rec.record_gf2p9_triple_b(
        &bi[2 * n16..3 * n16],
        &bii[2 * n16..3 * n16],
        &ai[..n16],
        &aii[..n16],
...
pub mod test {
    ...
    use crate::{
        ...
        util::mul_triple_vec::{
            MulTripleVector, NoMulTripleRecording, Ohv16TripleEncoder, Ohv16TripleVector,
            test::check_correct_encoding,
        },
        wollut16::{
            RndOhv16Output,
            offline::{
                generate_ohv16_bitslice_opt_check, generate_ohv16_with_bitstring_recording,
                generate_random_ohv16, generate_random_ohv16_bitstring,
                generate_random_ohv16_bitstring_mt, generate_random_ohv16_mt, un_bitslice,
            },
        },
    };
    ...
    fn check_correct_triples(
        mut triples1: MulTripleVector<GF2p64>,
        mut triples2: MulTripleVector<GF2p64>,
        mut triples3: MulTripleVector<GF2p64>,
    ) {
        for (t1, t2, t3) in izip!(
            triples1.drain_into_rss_iter(),
            triples2.drain_into_rss_iter(),
            triples3.drain_into_rss_iter()
        ) {
    ...
    fn ohv16_bitstring_correct() {
        ...
        let program = |b0: Vec<RssShare<BsBool16>>,
            ...
            move |p: &mut ChidaParty| {
                ...
                let ohv = generate_ohv16_with_bitstring_recording(
                    p.as_party_mut(),
                    &mut triples,
                    16,
                    b0,
    ...
    fn random_ohv16_bitstring_mt() {
        ...
        let program = || {
            |p: &mut ChidaParty| {
                let mut rec = MulTripleVector::new();
                let ohv =
                    generate_random_ohv16_bitstring_mt(p.as_party_mut(), &mut rec, N).unwrap();
                (ohv, rec)
            }
        };
    ...
    fn generate_ohv16_bitslice_opt_check_correct() {
        ...
        let program = |b0: Vec<RssShare<BsBool16>>,
            ...
            move |p: &mut ChidaParty| {
                ...
                let ohv = generate_ohv16_bitslice_opt_check(
                    p.as_party_mut(),
                    &mut triples,
                    &b0,
                    &b1,
        ...
        let (
            ((o1, rand1, mut triples1), _),
            ((o2, rand2, mut triples2), _),
            ((o3, rand3, mut triples3), _),
        ) = ChidaSetup::localhost_setup(
            program(bit0.0, bit1.0, bit2.0, bit3.0),
            program(bit0.1, bit1.1, bit2.1, bit3.1),
            program(bit0.2, bit1.2, bit2.2, bit3.2),
        ...
        check_correct_encoding(
            Ohv16TripleEncoder(&mut triples1),
            Ohv16TripleEncoder(&mut triples2),
            Ohv16TripleEncoder(&mut triples3),
            2 * 16,
        );
    }
}
