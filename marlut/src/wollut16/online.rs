...
use crate::{
    ...
    wollut16_malsec::online::{un_wol_bitslice_gf4, wol_bitslice_gf4},
};
...
pub fn lut_layer(party: &mut WL16Party, v: &[GF4]) -> MpcResult<(Vec<GF4>, Vec<GF4>)> {
    if party.prep_ohv.len() < v.len() {
        panic!(
            "Not enough pre-processed random one-hot vectors available. Use WL16Party::prepare_rand_ohv to generate them."
        );
    }
    ...
    let ci: Vec<_> = izip!(
        v.iter(),
        rnd_ohv,
        ArithmeticBlackBox::<GF4>::generate_alpha(&mut party.inner, v.len())
    )
...
fn ss_to_rss_layer(
    ...
    // Shares of zero
    let alphas = ArithmeticBlackBox::<GF4>::generate_alpha(&mut party.inner, xss_i.len());
    //
    izip!(x_i.iter_mut(), xss_i, alphas).for_each(|(y_i, &xss, alpha)| *y_i = xss + alpha);
    party
        .io()
...
mod test {
    ...
    use crate::rep3_core::{party::RngExt, share::RssShare, test::TestSetup};
    use crate::{
        aes::test::{
        ...
        wollut16::{
            WL16Party,
            online::{GF4_INV, gf8_inv_layer_opt, gf8_inv_layer_opt_mt},
            test::WL16Setup,
        },
    };
    ...
    fn lut_layer_correct() {
        ...
        let ((s1, p1), (s2, p2), (s3, p3)) =
            WL16Setup::localhost_setup(program(in1), program(in2), program(in3));
    ...
    fn gf8_inv_correct() {
        ...
        let ((s1, _), (s2, _), (s3, _)) =
            WL16Setup::localhost_setup(program(s1), program(s2), program(s3));
        assert_eq!(s1.len(), inputs.len());
        assert_eq!(s2.len(), inputs.len());
        assert_eq!(s3.len(), inputs.len());
        for (i, (s1, s2, s3)) in izip!(s1, s2, s3).enumerate() {
