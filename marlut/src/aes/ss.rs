...
pub fn aes128_no_keyschedule_mal<Protocol: GF8InvBlackBoxSSMal>(
    ...
    for r in 1..=9 {
        ...
        state_ss
            .s
            .iter_mut()
            .for_each(|dst| *dst = dst.aes_sbox_affine_transform() + c);
        ...
        y.si.iter_mut()
            .for_each(|si| *si = (*si + c_rss.si).inv_aes_sbox_affine_transform());
        y.sii
            .iter_mut()
            .for_each(|sii| *sii = (*sii + c_rss.sii).inv_aes_sbox_affine_transform());
    ...
    state_ss
        .s
        .iter_mut()
        .zip_eq(party.main_party_mut().generate_alpha(l))
        .for_each(|(s, alpha)| *s += alpha);
    let rcv = party
        .main_party_mut()
        .receive_field_slice(Direction::Next, &mut x_r.sii);
    party
        .main_party_mut()
        .send_field_slice(Direction::Previous, &state_ss.s);
    rcv.rcv()?;
    x_r.si = state_ss.s;
    ...
    state_rss
        .si
        .iter_mut()
        .for_each(|si| *si = si.aes_sbox_affine_transform() + c_rss.si);
    state_rss
        .sii
        .iter_mut()
        .for_each(|sii| *sii = sii.aes_sbox_affine_transform() + c_rss.sii);
...
pub fn random_state<Protocol: Party>(party: &mut Protocol, size: usize) -> VectorAesStateSS {
    let rand = party
        .generate_random(size * 16)
        .into_iter()
        .map(|rss| rss.si)
        .collect();
...
fn sbox_layer<Protocol: GF8InvBlackBoxSS>(party: &mut Protocol, s: &mut [GF8]) -> MpcResult<()> {
...
fn inv_sbox_layer<Protocol: GF8InvBlackBoxSS>(
    ...
    let c = party.constant(GF8(0x63));
    s.iter_mut()
        .for_each(|dst| *dst = (*dst + c).inv_aes_sbox_affine_transform());
...
pub mod test {
    ...
    use crate::aes::test::{
        AES_SBOX, AES128_TEST_EXPECTED_OUTPUT, AES128_TEST_INPUT, AES128_TEST_ROUNDKEYS,
        secret_share_aes_key_state,
    };
    use crate::share::Field;
    use crate::share::gf8::GF8;

    use super::{GF8InvBlackBoxSS, VectorAesStateSS, sbox_layer};

    pub fn secret_share_ss<R: Rng + CryptoRng, F: Field>(
        rng: &mut R,
        values: &[F],
    ) -> (Vec<F>, Vec<F>, Vec<F>) {
    ...
    fn secret_share_vectorstate_ss<R: Rng + CryptoRng>(
        rng: &mut R,
        state: &[GF8],
    ) -> (VectorAesStateSS, VectorAesStateSS, VectorAesStateSS) {
    ...
    pub fn test_sub_bytes_ss<S: TestSetup<P>, P: GF8InvBlackBoxSS>(
    ...
    pub fn test_aes128_no_keyschedule_gf8_ss<S: TestSetup<P>, P: GF8InvBlackBoxSS>(
        n_blocks: usize,
        n_worker_threads: Option<usize>,
    ) {
    ...
    pub fn test_inv_aes128_no_keyschedule_gf8_ss<S: TestSetup<P>, P: GF8InvBlackBoxSS>(
        n_blocks: usize,
        n_worker_threads: Option<usize>,
    ) {
