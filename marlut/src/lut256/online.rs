...
use crate::rep3_core::{
    ...
    party::{MainParty, error::MpcResult},
...
use crate::{
    ...
    chida::ChidaParty,
    share::{Field, gf8::GF8},
    util::{ArithmeticBlackBox, mul_triple_vec::NoMulTripleRecording},
};

use super::{LUT256Party, RndOhv256Output, lut256_tables, offline};
...
impl GF8InvBlackBox for LUT256Party {
    ...
    fn do_preprocessing(
        ...
        let mut prep =
            if self.inner.has_multi_threading() && 2 * n_prep > self.inner.num_worker_threads() {
                offline::generate_rndohv256_mt::<_, GF8>(
                    self.inner.as_party_mut(),
                    &mut NoMulTripleRecording,
                    n_prep,
                )?
            } else {
                offline::generate_rndohv256(
                    self.inner.as_party_mut(),
                    &mut NoMulTripleRecording,
                    n_prep,
                )?
    ...
    fn gf8_inv(&mut self, si: &mut [GF8], sii: &mut [GF8]) -> MpcResult<()> {
        ...
        if self.prep_ohv.len() < si.len() {
            panic!(
                "Not enough pre-processed random one-hot vectors available. Use LUT256Party::prepare_rand_ohv to generate them."
            );
        }
