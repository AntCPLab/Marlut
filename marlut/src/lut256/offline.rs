...
use crate::rep3_core::{
    party::{MainParty, Party, error::MpcResult},
    share::{HasZero, RssShare},
};
use crate::{
    chida,
    share::{Field, bs_bool16::BsBool16, gf8::GF8},
    util::{
        self,
        mul_triple_vec::{BitStringMulTripleRecorder, MulTripleRecorder, Ohv16TripleRecorder},
    },
    wollut16,
};
...
pub fn generate_rndohv256<Rec: BitStringMulTripleRecorder>(
    party: &mut MainParty,
...
where
    Rec::ThreadMulTripleRecorder: BitStringMulTripleRecorder,
{
    ...
    let thread_parties = party.create_thread_parties_with_additional_data(ranges, |start, end| {
        Some(mul_triple_recorder.create_thread_mul_triple_recorder(start, end))
    });
...
pub fn generate_rndohv256_ss<Rec: MulTripleRecorder<BsBool16>>(
    party: &mut MainParty,
    mul_triple_recorder: &mut Rec,
    amount: usize,
) -> MpcResult<Vec<RndOhv256OutputSS>> {
    let n_blocks = amount.div_ceil(16);
...
pub fn generate_rndohv256_ss_ohv_check<Rec: Ohv16TripleRecorder>(
    party: &mut MainParty,
    mul_triple_recorder: &mut Rec,
    amount: usize,
) -> MpcResult<Vec<RndOhv256OutputSS>> {
...
fn generate_rndohv256_ss_ohv_check_output<P: Party, Rec: Ohv16TripleRecorder>(
    party: &mut P,
    triple_rec: &mut Rec,
    n: usize,
    random_bits: [Vec<RssShare<BsBool16>>; 8],
) -> MpcResult<Vec<RndOhv256OutputSS>> {
    ...
    let lower = wollut16::offline::generate_ohv16_bitslice_opt_check(
        party, triple_rec, &r0, &r1, &r2, &r3,
    )?;
    let upper = wollut16::offline::generate_ohv16_bitslice_opt_check(
        party, triple_rec, &r4, &r5, &r6, &r7,
    )?;
...
pub fn generate_rndohv256_ss_mt<Rec: MulTripleRecorder<BsBool16>>(
    party: &mut MainParty,
    mul_triple_recorder: &mut Rec,
    amount: usize,
) -> MpcResult<Vec<RndOhv256OutputSS>> {
    ...
    party.run_in_threadpool(|| {
        thread_parties
            ...
            .map(|mut thread_party| {
                ...
                let mut recorder = thread_party.additional_data.take().unwrap();
                let out = generate_ohv256_ss_output(
                    &mut thread_party,
                    &mut recorder,
                    16 * n_blocks_per_thread,
                    bits,
                )
                .unwrap();
                (out, recorder)
...
pub fn generate_rndohv256_ss_ohv_check_mt<Rec: Ohv16TripleRecorder>(
    party: &mut MainParty,
    mul_triple_recorder: &mut Rec,
    amount: usize,
) -> MpcResult<Vec<RndOhv256OutputSS>> {
    let n_blocks = amount.div_ceil(16);
    let ranges = party.split_range_equally(n_blocks);
    let task_sizes = ranges
        .iter()
        .map(|(start, end)| 4 * (end - start))
        .collect_vec();
    let slices = mul_triple_recorder
        .create_thread_mul_triple_recorders(&task_sizes)
        .into_iter()
        .map(|sl| Some(sl))
        .collect();
    ...
    party.run_in_threadpool(|| {
        thread_parties
            ...
            .map(|mut thread_party| {
                ...
                generate_rndohv256_ss_ohv_check_output(
                    &mut thread_party,
                    &mut rec,
                    16 * n_blocks_per_thread,
                    bits,
...
fn random_8_bits<P: Party>(party: &mut P, n_blocks: usize) -> [Vec<RssShare<BsBool16>>; 8] {
    [
        party.generate_random(n_blocks),
        party.generate_random(n_blocks),
        party.generate_random(n_blocks),
        party.generate_random(n_blocks),
        party.generate_random(n_blocks),
...
fn tensor_two_ohv16<P: Party>(
    party: &mut P,
    lower: [Vec<RssShare<BsBool16>>; 16],
    upper: [Vec<RssShare<BsBool16>>; 16],
    random_bits: [Vec<RssShare<BsBool16>>; 8],
    n: usize,
) -> Vec<RndOhv256OutputSS> {
    // tensor prod
    let mut bits = Vec::with_capacity(256);
    for upper_bit in upper {
        bits.extend(lower.iter().map(|lower_bit| {
            let bit: Vec<BsBool16> = izip!(
                party.generate_alpha::<BsBool16>(lower_bit.len()),
                &upper_bit,
                lower_bit
            )
            .map(|(alpha, x, y)| alpha + x.si * y.si + (x.si + x.sii) * (y.si + y.sii))
            .collect_vec();
            bit
        }));
    }
    ...
    ohvs.into_iter()
        .zip(rand)
        .map(|(ohv, rand)| RndOhv256OutputSS {
            ohv,
            random_si: rand.si,
            random_sii: rand.sii,
        })
...
fn generate_ohv256_ss_output<P: Party, Rec: MulTripleRecorder<BsBool16>>(
    party: &mut P,
    triple_rec: &mut Rec,
    n: usize,
    random_bits: [Vec<RssShare<BsBool16>>; 8],
...
fn simple_mul<P: Party, Rec: BitStringMulTripleRecorder>(
    ...
    mul_triple_recorder.record_bit_bitstring_triple(
        msb.len(),
        &ai_bit,
        &aii_bit,
        &bi,
...
mod test {
    ...
    use crate::{
        ...
        lut256::{
            RndOhv,
            offline::{
                generate_ohv256_output, generate_ohv256_ss_output, generate_rndohv256,
                generate_rndohv256_mt, generate_rndohv256_ss, generate_rndohv256_ss_mt,
            },
        },
        share::{
            Field,
    ...
    fn generate_ohv2() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 2).unwrap()
            }
        };
    ...
    fn generate_ohv4() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 4).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn generate_ohv8() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 8).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn generate_ohv16() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 16).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn generate_ohv32() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 32).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn generate_ohv64() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 64).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn generate_ohv128() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 128).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn generate_ohv256() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv(p.as_party_mut(), &mut NoMulTripleRecording, share, 256).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn generate_ohv_output256() {
        ...
        let program = |share: Vec<Vec<RssShare<BsBool16>>>| {
            move |p: &mut ChidaParty| {
                generate_ohv256_output(p.as_party_mut(), &mut NoMulTripleRecording, share).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup(program(shares.0), program(shares.1), program(shares.2));
    ...
    fn rnd_ohv256() {
        ...
        let program = || {
            |p: &mut ChidaParty| {
                generate_rndohv256(p.as_party_mut(), &mut NoMulTripleRecording, N).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
    ...
    fn rnd_ohv256_mt() {
        ...
        let program = || {
            |p: &mut ChidaParty| {
                generate_rndohv256_mt::<_, BsBool16>(p.as_party_mut(), &mut NoMulTripleRecording, N)
                    .unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
    ...
    fn test_rndohv256_registers_correct_triples() {
        ...
        let ((mut rec1, _), (mut rec2, _), (mut rec3, _)) =
            ChidaSetup::localhost_setup(program(), program(), program());
        ...
        izip!(
            rec1.drain_into_rss_iter(),
            rec2.drain_into_rss_iter(),
            rec3.drain_into_rss_iter()
        )
        .for_each(|(trip1, trip2, trip3)| {
    ...
    fn ohv256_ss_output() {
        ...
        let program = |share: [Vec<RssShare<BsBool16>>; 8]| {
            move |p: &mut ChidaParty| {
                generate_ohv256_ss_output(
                    p.as_party_mut(),
                    &mut NoMulTripleRecording,
                    256 * 16,
                    share,
                )
                .unwrap()
    ...
    fn rnd_ohv256_ss() {
        ...
        let program = || {
            move |p: &mut ChidaParty| {
                generate_rndohv256_ss(p.as_party_mut(), &mut NoMulTripleRecording, N).unwrap()
            }
        };
    ...
    fn rnd_ohv256_ss_mt() {
        ...
        let program = || {
            |p: &mut ChidaParty| {
                generate_rndohv256_ss_mt(p.as_party_mut(), &mut NoMulTripleRecording, N).unwrap()
            }
        };
        let ((ohv1, _), (ohv2, _), (ohv3, _)) =
            ChidaSetup::localhost_setup_multithreads(N_THREADS, program(), program(), program());
