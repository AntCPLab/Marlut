...
use crate::rep3_core::{
    network::{ConnectedParty, task::Direction},
    party::{
        MainParty, Party,
...
use crate::{
    aes::ss::{GF8InvBlackBoxSS, GF8InvBlackBoxSSMal},
    share::{
        bs_bool16::BsBool16,
        gf2p64::{GF2p64, GF2p64InnerProd, GF2p64Subfield},
        gf8::GF8,
    },
    util::mul_triple_vec::{
        BsBool16Encoder, MulTripleEncoder, MulTripleRecorder, MulTripleVector,
        NoMulTripleRecording, Ohv16TripleEncoder, Ohv16TripleVector,
    },
    wollut16_malsec::mult_verification,
};

use super::{RndOhv256OutputSS, lut256_tables, offline};
...
impl GF8InvBlackBoxSS for Lut256SSParty {
    fn constant(&self, value: GF8) -> GF8 {
        if self.inner.i == 0 { value } else { GF8::ZERO }
    }
    ...
    fn do_preprocessing(&mut self, n_keys: usize, n_blocks: usize) -> MpcResult<()> {
        ...
        let mut prep = if self.inner.has_multi_threading()
            && 2 * n_prep > self.inner.num_worker_threads()
        {
    ...
    fn gf8_inv(&mut self, s: &mut [GF8]) -> MpcResult<()> {
        ...
        if self.prep_ohv.len() < n {
            panic!(
                "Not enough pre-processed random one-hot vectors available. Use LUT256SSParty::do_preprocessing to generate them."
            );
        }
        ...
        izip!(
            s.iter_mut(),
            self.inner.generate_alpha::<GF8>(n),
            rnd_ohv.iter()
        )
        .for_each(|(dst, alpha, ohv)| *dst += alpha + ohv.random_si);
        self.inner
            .io()
            .send_field::<GF8>(Direction::Next, s.iter(), n);
        self.inner
            .io()
            .send_field::<GF8>(Direction::Previous, s.iter(), n);
        ...
        if self.inner.has_multi_threading() && 2 * n > self.inner.num_worker_threads() {
            ...
            self.inner.run_in_threadpool(|| {
                s.par_chunks_mut(chunk_size)
                    .zip_eq(rnd_ohv.par_chunks(chunk_size))
                    .for_each(|(s_chunk, ohv_chunk)| {
                        s_chunk.iter_mut().zip(ohv_chunk).for_each(|(dst, ohv)| {
                            *dst = ohv
                                .ohv
                                .lut(dst.0 as usize, &lut256_tables::GF8_INV_BITSLICED_LUT);
                        });
                    });
                Ok(())
            })?;
    ...
    fn output(&mut self, data: &[GF8]) -> MpcResult<Vec<GF8>> {
        let rcv_data_ii = self
            .inner
            .io()
            .receive_field::<GF8>(Direction::Next, data.len());
        let rcv_data_iii = self
            .inner
            .io()
            .receive_field::<GF8>(Direction::Previous, data.len());
        self.inner
            .io()
            .send_field::<GF8>(Direction::Next, data.iter(), data.len());
        self.inner
            .io()
            .send_field::<GF8>(Direction::Previous, data.iter(), data.len());

        let data_ii = rcv_data_ii.rcv()?;
        let data_iii = rcv_data_iii.rcv()?;
        let res = izip!(data, data_ii, data_iii)
            .map(|(&si, sii, siii)| si + sii + siii)
            .collect();
...
impl GF8InvBlackBoxSSMal for Lut256SSMalParty {
    ...
    fn do_preprocessing(&mut self, n_keys: usize, n_blocks: usize) -> MpcResult<()> {
        ...
        match &mut self.prep_triples {
            PrepTriples::GF2(gf2_triples) => {
                gf2_triples.reserve_for_more_triples((n_prep * 22) / 16)
            } // 22 multiplications per S-box packed in 16
            PrepTriples::Ohv(ohv_triples) => {
                ohv_triples.reserve_for_more_triples((n_prep * 4) / 16)
            } // 4 multiplications per S-box packed in 16
        }
            ...
            && 2 * n_prep > self.inner.num_worker_threads()
        {
            match &mut self.prep_triples {
                PrepTriples::GF2(gf2_triples) => {
                    offline::generate_rndohv256_ss_mt(&mut self.inner, gf2_triples, n_prep)?
                }
                PrepTriples::Ohv(ohv_tripes) => offline::generate_rndohv256_ss_ohv_check_mt(
                    &mut self.inner,
                    ohv_tripes,
                    n_prep,
                )?,
            ...
            match &mut self.prep_triples {
                PrepTriples::GF2(gf2_triples) => {
                    offline::generate_rndohv256_ss(&mut self.inner, gf2_triples, n_prep)?
                }
                PrepTriples::Ohv(ohv_triples) => {
                    offline::generate_rndohv256_ss_ohv_check(&mut self.inner, ohv_triples, n_prep)?
                }
            }
        };
        if self.prep_ohv.is_empty() {
    ...
    fn gf8_inv_and_rss_output(
        ...
        if self.prep_ohv.len() < n {
            panic!(
                "Not enough pre-processed random one-hot vectors available. Use Lut256SSMalParty::do_preprocessing to generate them."
            );
        }
        ...
        let rcv_ciii = self
            .inner
            .io()
            .receive_field_slice(Direction::Previous, out_ii);
        ...
        s.iter_mut()
            .zip(out_i.iter())
            .for_each(|(dst, cii)| *dst += *cii);
        rcv_ciii.rcv()?;
        s.iter_mut()
            .zip(out_ii.iter())
            .for_each(|(dst, ciii)| *dst += *ciii);

        izip!(out_i.iter_mut(), s.iter(), rnd_ohv).for_each(|(dst, c, r)| *dst = *c + r.random_si);
        izip!(out_ii.iter_mut(), s.iter(), rnd_ohv)
            .for_each(|(dst, c, r)| *dst = *c + r.random_sii);
        if self.inner.has_multi_threading() && 2 * n > self.inner.num_worker_threads() {
            let ranges = self.inner.split_range_equally(n);
            let chunk_size = ranges[0].1 - ranges[0].0;
    ...
    fn gf8_inv_rss_to_ss(&mut self, out: &mut [GF8], si: &[GF8], sii: &[GF8]) -> MpcResult<()> {
        ...
        let rnd_ohv = &self.prep_ohv[self.prep_ohv.len() - n..];
        izip!(out.iter_mut(), si.iter(), rnd_ohv.iter())
            .for_each(|(dst, si, r)| *dst = *si + r.random_si);
        let tmp = sii
            .iter()
            .zip_eq(rnd_ohv.iter())
            .map(|(sii, r)| *sii + r.random_sii)
            .collect_vec();
        ...
        if self.inner.has_multi_threading() && 2 * n > self.inner.num_worker_threads() {
            ...
            self.inner.run_in_threadpool(|| {
                out.par_chunks_mut(chunk_size)
                    ...
                    .zip_eq(rnd_ohv.par_chunks(chunk_size))
                    .for_each(|((out_chunk, v_chunk), ohv_chunk)| {
                        izip!(out_chunk.iter_mut(), v_chunk, ohv_chunk).for_each(
                            |(dst, v, ohv)| {
                                *dst = ohv
                                    .ohv
                                    .lut(v.0 as usize, &lut256_tables::GF8_INV_BITSLICED_LUT);
                            },
                        );
                    });
                Ok(())
    ...
    fn gf8_inv_rss(&mut self, si: &mut [GF8], sii: &mut [GF8]) -> MpcResult<()> {
        debug_assert_eq!(si.len(), sii.len());
        let n = si.len();
        if self.prep_ohv.len() < n {
            panic!(
        ...
        let rnd_ohv = &self.prep_ohv[self.prep_ohv.len() - n..];
        izip!(si.iter_mut(), rnd_ohv.iter()).for_each(|(dst, r)| *dst += r.random_si);
        izip!(sii.iter_mut(), rnd_ohv.iter()).for_each(|(dst, r)| *dst += r.random_sii);

        let v = self.inner.open_rss(&mut self.context, si, sii)?;

        let recv_data = self.inner.receive_field_slice(Direction::Next, sii);
        ...
        if self.inner.has_multi_threading() && 2 * n > self.inner.num_worker_threads() {
            ...
            izip!(si.iter_mut(), v, rnd_ohv).for_each(|(dst, v, ohv)| {
                *dst = ohv
                    .ohv
                    .lut(v.0 as usize, &lut256_tables::GF8_INV_BITSLICED_LUT);
            });
        ...
        self.inner
            .send_field::<GF8>(Direction::Previous, si.iter(), si.len());
        recv_data.rcv()?;
    ...
    fn finalize(&mut self) -> MpcResult<()> {
        ...
        let mut sbox_pairs_encoder =
            SboxPairEncoder::new(&mut self.x_i, &mut self.x_ii, &mut self.y_i, &mut self.y_ii);
        let res = if self.inner.has_multi_threading() {
            match &mut self.prep_triples {
                PrepTriples::GF2(gf2_triples) => {
                    mult_verification::verify_multiplication_triples_mt(
                        &mut self.inner,
                        &mut self.context,
                        &mut [&mut BsBool16Encoder(gf2_triples), &mut sbox_pairs_encoder],
                        false,
                    )
                }
                PrepTriples::Ohv(ohv_triples) => {
                    mult_verification::verify_multiplication_triples_mt(
                        ...
                        &mut self.context,
                        &mut [
                            &mut Ohv16TripleEncoder(ohv_triples),
                            &mut sbox_pairs_encoder,
                        ],
                        false,
                    )
                }
            ...
            match &mut self.prep_triples {
                PrepTriples::GF2(gf2_triples) => mult_verification::verify_multiplication_triples(
                    &mut self.inner,
                    &mut self.context,
                    &mut [&mut BsBool16Encoder(gf2_triples), &mut sbox_pairs_encoder],
                    false,
                ),
                PrepTriples::Ohv(ohv_triples) => mult_verification::verify_multiplication_triples(
                    &mut self.inner,
                    &mut self.context,
                    &mut [
                        &mut Ohv16TripleEncoder(ohv_triples),
...
impl<'a> MulTripleEncoder for SboxPairEncoder<'a> {
    ...
    fn add_triples(
        ...
        Self::add_triples_local(
            &self.x_i, &self.x_ii, &self.y_i, &self.y_ii, x, y, zi, zii, weight, rand,
        );
    }
...
#[cfg(test)]
mod test {
    use crate::aes::ss::test::{
        secret_share_ss, test_aes128_no_keyschedule_gf8_ss, test_inv_aes128_no_keyschedule_gf8_ss,
        test_sub_bytes_ss,
    };
    use crate::aes::ss::{GF8InvBlackBoxSS, GF8InvBlackBoxSSMal, aes128_no_keyschedule_mal};
    use crate::aes::test::{
        AES128_TEST_EXPECTED_OUTPUT, AES128_TEST_INPUT, AES128_TEST_ROUNDKEYS,
        secret_share_aes_key_state, secret_share_vectorstate,
    };
    use crate::aes::{AesKeyState, INV_GF8, VectorAesState};
    ...
    use crate::rep3_core::test::{TestSetup, localhost_connect};
    ...
    >(
        ...
        fn adapter<T, Fx: FnOnce(&mut Lut256SSMalParty) -> T>(
            ...
            let mut party =
                Lut256SSMalParty::setup(conn, use_ohv_check, n_worker_threads, None).unwrap();
            let t = f(&mut party);
            party.finalize().unwrap();
            party.inner.teardown().unwrap();
            (t, party)
    ...
    fn gf8_inv_rss_to_ss() {
        ...
        let program = |input: Vec<RssShare<GF8>>| {
            move |p: &mut Lut256SSMalParty| {
                ...
                let (xi, xii): (Vec<_>, Vec<_>) =
                    input.into_iter().map(|rss| (rss.si, rss.sii)).unzip();
        ...
        let ((y1, _), (y2, _), (y3, _)) =
            localhost_setup_lut256_ss_mal(program(in0), program(in1), program(in2), false, None);
    ...
    fn gf8_inv_rss() {
        ...
        let program = |input: Vec<RssShare<GF8>>| {
            move |p: &mut Lut256SSMalParty| {
                p.do_preprocessing(0, 256 / 16).unwrap();
                let (mut xi, mut xii): (Vec<_>, Vec<_>) =
                    input.into_iter().map(|rss| (rss.si, rss.sii)).unzip();
                p.gf8_inv_rss(&mut xi, &mut xii).unwrap();
                let input_rss = xi
                    .into_iter()
                    .zip_eq(xii)
        ...
        let ((input_rss1, _), (input_rss2, _), (input_rss3, _)) =
            localhost_setup_lut256_ss_mal(program(in0), program(in1), program(in2), false, None);
    ...
    fn gf8_inv_and_rss_output() {
        ...
        let program = |mut input: Vec<GF8>| {
            move |p: &mut Lut256SSMalParty| {
                ...
                p.gf8_inv_and_rss_output(&mut input, &mut input_i, &mut input_ii)
                    .unwrap();
                let input_rss = input_i
                    .into_iter()
                    .zip_eq(input_ii)
                    .map(|(si, sii)| RssShare::from(si, sii))
                    .collect_vec();
        ...
        let (((y1, input_rss1), _), ((y2, input_rss2), _), ((y3, input_rss3), _)) =
            localhost_setup_lut256_ss_mal(program(in0), program(in1), program(in2), false, None);
    ...
    fn test_aes128_no_keyschedule_gf8_malss<S: TestSetup<P>, P: GF8InvBlackBoxSSMal>(
        n_blocks: usize,
        n_worker_threads: Option<usize>,
    ) {
        ...
        for i in 0..11 {
            let (s1, s2, s3) =
                secret_share_aes_key_state(&mut rng, &AES128_TEST_ROUNDKEYS[i].map(|x| GF8(x)));
            ks1.push(s1);
            ks2.push(s2);
            ks3.push(s3);
        }
