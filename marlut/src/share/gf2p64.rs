...
use std::{
    arch::x86_64::{__m128i, _mm_clmulepi64_si128, _mm_xor_si128},
    borrow::Borrow,
    fmt::{Debug, Formatter},
    iter::Sum,
...
use super::{
    Empty, Field, FieldLike, HasTwo, InnerProduct, Invertible, bs_bool16::BsBool16, gf4::GF4,
    gf8::GF8,
};
...
impl GF2p64 {
    ...
    pub fn fast_clmul_inner_product(a: &[Self], b: &[Self]) -> Self {
        ...
        let wrdcar = a.iter().zip(b).fold(0, |wrdcar, (a, b)| unsafe {
    ...
    pub fn fast_clmul_inner_product(a: &[Self], b: &[Self]) -> Self {
        use core::arch::x86_64::{
            __m128i, _mm_clmulepi64_si128, _mm_set_epi64x, _mm_setzero_si128, _mm_storeu_si128,
            _mm_xor_si128,
        };
        let wrdcar: __m128i = unsafe {
            a.iter().zip(b).fold(_mm_setzero_si128(), |wrdcar, (a, b)| {
    ...
    pub fn extend_from_bitstring(vec: &mut Vec<Self>, bitstring: &[BsBool16], simd_len: usize) {
        ...
        bitstring
            .chunks_exact(simd_len)
            .enumerate()
            .for_each(|(bitpos, simd_bit)| {
    ...
    fn mul_inner(a: &GF2p64, b: &GF2p64) -> GF2p64 {
    ...
    pub fn mul_multiple(c: &mut [GF2p64], a: &[GF2p64], b: &[GF2p64]) {
        use std::arch::x86_64::{
            _mm_set_epi64x, _mm512_broadcast_i64x2, _mm512_clmulepi64_epi128,
            _mm512_mask_compressstoreu_epi64, _mm512_maskz_expandloadu_epi64,
            _mm512_ternarylogic_epi64,
        };
        debug_assert!(c.len() % 4 == 0);

        unsafe {
            let rr = _mm512_broadcast_i64x2(_mm_set_epi64x(0, IRREDUCIBLE_POLYNOMIAL as i64));
            for i in (0..c.len()).step_by(4) {
                let a = _mm512_maskz_expandloadu_epi64(
                    0b01010101,
                    &a[i] as *const GF2p64 as *const i64,
                );
                let b = _mm512_maskz_expandloadu_epi64(
                    0b01010101,
                    &b[i] as *const GF2p64 as *const i64,
                );
                let output = _mm512_clmulepi64_epi128::<LOWER_LOWER>(a, b);
                let output_red = _mm512_clmulepi64_epi128::<UPPER_LOWER>(output, rr);
                let output_red_2 = _mm512_clmulepi64_epi128::<UPPER_LOWER>(output_red, rr);
                let output = _mm512_ternarylogic_epi64::<0x96>(output, output_red, output_red_2);
                _mm512_mask_compressstoreu_epi64(
                    &mut c[i] as *mut GF2p64 as *mut i64,
                    0b01010101,
                    output,
                );
            }
    ...
    pub fn inner_product_avx512(a: &[GF2p64], b: &[GF2p64]) -> GF2p64 {
        use std::arch::x86_64::{
            _mm_cvtsi128_si64, _mm_cvtsi128_si64x, _mm_extract_epi64, _mm_set_epi64x,
            _mm_shuffle_epi32, _mm256_castsi256_si128, _mm256_extracti128_si256, _mm256_xor_si256,
            _mm512_castsi512_si256, _mm512_clmulepi64_epi128, _mm512_extracti64x4_epi64,
            _mm512_load_epi64, _mm512_maskz_expandloadu_epi64, _mm512_setzero_epi32,
            _mm512_ternarylogic_epi64, _mm512_unpacklo_epi64, _mm512_xor_epi64,
        };
        ...
        unsafe {
            ...
            for i in (0..a.len()).step_by(8) {
                let a = _mm512_load_epi64(&a[i] as *const GF2p64 as *const i64);
                let b = _mm512_load_epi64(&b[i] as *const GF2p64 as *const i64);
                let result_low = _mm512_clmulepi64_epi128::<LOWER_LOWER>(a, b);
                let result_high = _mm512_clmulepi64_epi128::<UPPER_UPPER>(a, b);
                sum = _mm512_ternarylogic_epi64::<0x96>(sum, result_low, result_high);
            }

            // Since there is no xor reduce intrinsic, the following is based on what Clang does for add reduce
            let high = _mm512_extracti64x4_epi64::<1>(sum);
            let sum = _mm256_xor_si256(high, _mm512_castsi512_si256(sum));
            let high = _mm256_extracti128_si256::<1>(sum);
            let sum = _mm_xor_si128(high, _mm256_castsi256_si128(sum));
            let word = _mm_cvtsi128_si64(sum);
            let carry = _mm_extract_epi64::<1>(sum);
            Self::propagate_carries(word as u64, carry as u64)
        }
    }
}

impl BasicFieldLike for GF2p64 {}

impl FieldLike for GF2p64 {
...
impl Add<Empty> for GF2p64 {
    type Output = Self;
    fn add(self, _rhs: Empty) -> Self::Output {
        self
    }
}
impl Sub<Empty> for GF2p64 {
    type Output = Self;
    fn sub(self, _rhs: Empty) -> Self::Output {
        self
    }
}
impl Mul<Empty> for GF2p64 {
    type Output = Empty;
...
pub fn embed_gf4p4_deg3(a0: GF4, a1: GF4, a2: GF4) -> GF2p64 {
    GF2p64(
        GF4_EXT_EB_TABLE0[a0.as_u8() as usize]
            ^ GF4_EXT_EB_TABLE1[a1.as_u8() as usize]
            ^ GF4_EXT_EB_TABLE2[a2.as_u8() as usize],
    )
}
...
mod test {
    ...
    use crate::share::{
        Field, InnerProduct, Invertible, bs_bool16::BsBool16, gf2p64::GF2p64Subfield, gf4::GF4,
        gf8::GF8,
    };
    ...
    fn test_fast_mul() {
        ...
        a.into_iter()
            .zip(b)
            .for_each(|(a, b)| assert_eq!(a * b, GF2p64::mul_clmul_u64_fast(&a, &b)));
    }
    ...
    fn test_mul_multiple() {
        ...
        let expected = a
            ...
            .map(|(a, b)| *a * *b)
            .collect::<Vec<_>>();
        let mut actual = GF2p64::generate(&mut rng, 128);
        GF2p64::mul_multiple(&mut actual, &a, &b);
        assert_eq!(expected, actual);
    }
    ...
    fn test_new_inner_product() {
        let mut rng = thread_rng();
        let mut a = Vec::new_in(AlignedAllocator::<64>);
        a.resize(128, GF2p64::ZERO);
        let mut b = Vec::new_in(AlignedAllocator::<64>);
        b.resize(128, GF2p64::ZERO);

        GF2p64::fill(&mut rng, &mut a);
        GF2p64::fill(&mut rng, &mut b);
        let expected = GF2p64::inner_product(&a, &b);
        let actual = GF2p64::inner_product_avx512(&a, &b);
        assert_eq!(expected, actual);
    }
}
