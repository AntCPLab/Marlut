...
pub fn output_round<F: Field>(
    ...
    let (my, siii) = match party.i {
        0 => {
            ...
            let s3 = party
                .io()
                .receive_field::<F>(Direction::Previous, to_p1.len())
                .rcv()?;
            (to_p1, s3)
        }
        1 => {
...
pub fn mul_no_sync<P: Party, F: Field>(
    ...
    let alphas = party.generate_alpha::<F>(ci.len());
...
pub mod test {
    use crate::aes::test::{
        ...
        test_aes256_no_keyschedule_gf8, test_inv_aes128_no_keyschedule_gf8, test_sub_bytes,
    };
    use crate::chida::online::{
        VectorAesState, input_round, input_round_aes_states, mul, output_round,
    };
    ...
    use crate::rep3_core::test::{PartySetup, TestSetup, localhost_connect};
    use crate::share::gf8::GF8;
    ...
    ) -> ((T1, ChidaParty), (T2, ChidaParty), (T3, ChidaParty)) {
    ...
    >(
        ...
        fn adapter<T, Fx: FnOnce(&mut ChidaBenchmarkParty) -> T>(
            ...
            let mut party =
                ChidaBenchmarkParty::setup(conn, variant, n_worker_threads, None).unwrap();
            let t = f(&mut party);
